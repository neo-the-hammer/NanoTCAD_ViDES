...
// as well as the transmission coefficient, within the real space approach.
//
...
static PyObject* py_CNT_charge_T(PyObject* self, PyObject* args)
{
  ...
  if (!rank) printf("*********************************\n");
  if (!rank) printf("*** Computing NEGF in CNT *******\n"); 
  if (!rank) printf("*********************************\n\n");
  ...
  for (i=0;i<(Nc-1);i++)
    UPDIAG[i]=create_updiag(i,n,thop);
  ...
  for (i=1;i<Nc;i++)
    LOWDIAG[i]=create_lowdiag(i-1,n,thop);
